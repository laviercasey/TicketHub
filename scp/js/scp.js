//Copyright (c) 2007 TicketHub.com
...
function getCannedResponse(idx,fObj,target)
{
    ...
    Http.get({
        url: "dispatch.php?api=kbase&f=cannedResp&id="+idx+'&tid='+tid,
...
{
    // Bypass flag set by thConfirm callback
    if (formObj._thConfirmed) {
        delete formObj._thConfirmed;
        return true;
    }
	...
	if (max>0 && total > max )
 	{
 		alert("Максимальное количество: " + max + ".\nВыбрано: " + total + ".\nУберите " + (total-max) + " выделение(й).")
 		return (false);
 	}

 	if (total< min )
 	{
 		alert("Необходимо выбрать минимум " + min + " запись(ей). Выбрано: " + total + ".")
 		return (false);
 	}

  if(sure){
    // Use custom modal if available (avoids reset_all bug with async confirm)
    if(typeof window.thConfirm === 'function') {
        window.thConfirm("Будет обработано записей: " + total + ".\nПродолжить?", function() {
            formObj._thConfirmed = true;
            var btn = formObj.querySelector('input[type="submit"], button[type="submit"]');
            if (btn) btn.click(); else formObj.submit();
        }, function() {
            reset_all(formObj);
        });
        return false;
