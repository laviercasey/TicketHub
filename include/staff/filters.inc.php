...
<div id="search" style="display:<?=$showadd?'none':'block'?>;">
    ...
    <form action="admin.php?t=settings" method="GET" class="flex items-center gap-3">
        ...
        <span class="text-sm text-gray-600">Запрос:</span>
        <input type="text" name="query" value="<?=Format::htmlchars($_REQUEST['query'])?>" class="input w-64">
...
<div id="add" style="display:<?=$showadd?'block':'none'?>;">
    <div class="flex items-center gap-3 mb-4">
    <form action="admin.php?t=settings" method="POST" class="flex items-center gap-3">
    <?php echo Misc::csrfField(); ?>
    <input type="hidden" name="t" value="banlist">
    <input type="hidden" name="a" value="add">
    <span class="text-sm text-gray-600">Email:</span>
    <input type="text" name="email" value="<?=Format::htmlchars($_POST['email'])?>" class="input w-64">
    <button type="submit" name="add" class="btn-primary btn-sm"><i data-lucide="plus-circle" class="w-4 h-4"></i> Добавить</button>
    <a href="#" class="text-blue-600 hover:text-blue-800 text-sm" onClick="showHide('add','search'); return false;">(Найти)</a>
    </form>
    </div>
</div>
...
<div class="table-wrapper">
    <table class="table-modern">
        ...
        if($banlist && db_num_rows($banlist)):
            ...
            <tr><td colspan="4" class="table-td text-center text-gray-500">Запрос вернул пустой результат &nbsp;&nbsp;<a href="admin.php?t=banlist" class="text-blue-600 hover:text-blue-800">Начало</a></td></tr>
        <?
        endif; ?>
        </tbody>
    </table>
...
<div class="card-footer">
    ...
    <div class="flex items-center justify-center gap-3">
        <button class="btn-danger btn-sm" type="submit" name="delete" value="Удалить из листа"
                 onClick='return confirm("Вы уверены что хотите УДАЛИТЬ выбранный email из банлиста?");'>
            <i data-lucide="trash-2" class="w-4 h-4"></i> Удалить из листа
        </button>
    </div>
</div>
<?
