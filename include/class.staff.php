...
class Staff {
    ...
    function check_passwd($password){
        ...
        if(strpos($this->passwd, '$2y$') === 0) {
            return password_verify($password, $this->passwd);
        }

        if(hash_equals($this->passwd, md5($password))) {
            $newhash = password_hash($password, PASSWORD_DEFAULT);
            db_query('UPDATE '.STAFF_TABLE.' SET passwd='.db_input($newhash).', change_passwd=1 WHERE staff_id='.db_input($this->id));
            $this->passwd = $newhash;
            return TRUE;
        }
    ...
    function getDepts(){
        return array_filter(array_unique(array_merge(explode(',',$this->udata['dept_access']),array($this->dept_id))));
    }
    ...
    static function save($id,$vars,&$errors) {
        ...
        if($vars['npassword'] || $vars['vpassword'] || !$id){
            ...
            elseif($vars['npassword'] && strlen($vars['npassword'])<12)
                $errors['npassword']='Пароль должен содержать не менее 12 символов';
        }
        ...
        if(!$errors){
            ...
            if($vars['npassword'])
                $sql.=',passwd='.db_input(password_hash($vars['npassword'], PASSWORD_DEFAULT));
