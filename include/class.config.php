...
class Config {
    ...
    public $id=0;
    public $mysqltzoffset=0;
    public $config=array();

    public $defaultDept;
    public $defaultEmail;
    public $alertEmail;
    public $defaultSMTPEmail;
    ...
    function getPipeToken(){
        return $this->config['pipe_token'];
    }

    function getVersion(){
        return $this->config['thversion'] ?? $this->config['ostversion'] ?? '';
    }
    ...
    function updatePref($var,&$errors) {
        ...
        $f=array();
        $f['helpdesk_url']=array('type'=>'string',   'required'=>1, 'error'=>'Требуется URL системы');
        $f['helpdesk_title']=array('type'=>'string',   'required'=>1, 'error'=>'Требуется название системы');
        $f['default_dept_id']=array('type'=>'int',   'required'=>1, 'error'=>'Требуется отдел по умолчанию');
        $f['default_email_id']=array('type'=>'int',   'required'=>1, 'error'=>'Требуется email по умолчанию');
        $f['default_template_id']=array('type'=>'int',   'required'=>1, 'error'=>'Необходимо выбрать шаблон.');
        $f['staff_session_timeout']=array('type'=>'int',   'required'=>1, 'error'=>'Введите время простоя в минутах');
        $f['client_session_timeout']=array('type'=>'int',   'required'=>1, 'error'=>'Введите время простоя в минутах');
        $f['time_format']=array('type'=>'string',   'required'=>1, 'error'=>'Требуется формат времени');
        $f['date_format']=array('type'=>'string',   'required'=>1, 'error'=>'Требуется формат даты');
        $f['datetime_format']=array('type'=>'string',   'required'=>1, 'error'=>'Требуется формат даты и времени');
        $f['daydatetime_format']=array('type'=>'string',   'required'=>1, 'error'=>'Требуется формат дня, даты и времени');
        $f['admin_email']=array('type'=>'email',   'required'=>1, 'error'=>'Требуется действительный email');
        $f['autolock_minutes']=array('type'=>'int',   'required'=>1, 'error'=>'Введите время блокировки в минутах');
        ...
        if($var['ticket_alert_active']
            ...
            $errors['ticket_alert_active']='Не выбраны получатели';
        }
        if($var['message_alert_active']
            ...
            $errors['message_alert_active']='Не выбраны получатели';
        }
        ...
        if($var['note_alert_active']
            ...
            $errors['note_alert_active']='Не выбраны получатели';
        }

        if($var['strip_quoted_reply'] && !$var['reply_separator'])
            $errors['reply_separator']='Требуется разделитель ответов (?)';

        if($var['enable_captcha']){
            if (!extension_loaded('gd'))
                $errors['enable_captcha']='Требуется расширение GD';
            elseif(!function_exists('imagepng'))
                $errors['enable_captcha']='Для капчи требуется поддержка PNG';
        }

        if(!$errors['admin_email'] && Email::getIdByEmail($var['admin_email']))
            $errors['admin_email']='Email уже настроен как системный';
